...
// Create a new order
export const createOrder = (cartItems,shippingAddress) => async (dispatch, getState) => {
    try {
...
export const updateOrder = (orderId, updatedData) => async (dispatch, getState) => {
    try {
        ...
        const token = getState().auth?.token; // Ensure token exists
...
export const getAllOrdersForAdmin=()=>async(dispatch,getState)=>{
    try{
        ...
        dispatch({ type: FETCH_ADMIN_ORDERS_SUCCESS, payload: data.data });
    }catch(error){
        dispatch({ 
