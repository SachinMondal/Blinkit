  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      <header className="flex justify-between items-center p-4 bg-green-500 mt-2 rounded-b-xl text-white shadow sticky top-0 z-30">
        <h1 className="text-xs sm:text-base md:text-lg font-semibold tracking-wide break-all">
          Order ID: #{order._id}
        </h1>
        <Link
          to="/admin/orders"
          className="bg-green-100 hover:bg-green-200 text-green-700 rounded-md px-3 py-1.5 sm:px-4 sm:py-2 font-medium shadow text-xs sm:text-xs"
        >
          &larr; Back
        </Link>
      </header>
    ...
    <div className="flex flex-col items-center gap-2">
      ...
      <p className="text-gray-600">Fetching order details...</p>
        ...
        <>
          <main className="flex-grow container mx-auto px-3 py-4 max-w-7xl mb-28 md:mb-20">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6">
              <section className="md:col-span-2 bg-white rounded-lg shadow-md p-4 sm:p-6 flex flex-col gap-4 sm:gap-6">
...
${order.shippingAddress.city}, ${order.shippingAddress.state} - ${order.shippingAddress.zipCode}`}
                ...
                {order?.user?.locationPin && (
                  <div>
                    ...
                    <div className="h-60 w-full rounded-lg overflow-hidden border border-gray-300 shadow-sm">
                      <AdminUserMap
              ...
              <section className="bg-white rounded-lg shadow-md p-4 sm:p-6 flex flex-col">
                ...
                <div className="divide-y divide-gray-200 flex-grow overflow-auto max-h-[400px]">
                  {order?.orderItems?.map((item, index) => (
                    <div key={index} className="flex justify-between py-3 items-center gap-2">
                      <div>
          ...
          {/* Bottom Buttons */}
          {!["DELIVERED", "REJECT", "CANCEL"].includes(order.orderStatus) && (
            <footer className="fixed bottom-0 left-0 w-full bg-white p-4 flex flex-col md:flex-row justify-between gap-3 md:gap-4 shadow-lg border-t border-gray-200 z-50">
              <motion.button
